...
class Pelota(object):
    ...
    def pasar(self, jugador):
        ...
        arribo = globales.cuanto + 30
        self.destino = jugador.get_pos(arribo)
        self.paso = (self.destino - self.posicion) / 30.
...
def programa():
    ...
    while True:# globales.cuanto < 601:
                    ...
                    if event.key == K_UP:
                        if pelota.estado == Pelota.GIRANDO:
                            ...
                            while jugadores[rand].desfasaje == pelota.desfasaje:
                                rand = randrange(0, globales.nro_jugadores, 1)
                            print rand
                            print pelota.get_pos(globales.cuanto)
                            arribo = 60
            ...
            if globales.cuanto == arroja:
                rand = randrange(0, globales.nro_jugadores, 1)
                while jugadores[rand].desfasaje == pelota.desfasaje:
                    rand = randrange(0, globales.nro_jugadores, 1)
                arribo = 60
                pelota.pasar(jugadores[rand])
                arroja = globales.cuanto + randrange(45, 60, 1)
                if jugadores[rand].color == BLANCO:
                    globales.pases = globales.pases + 1
                    print globales.pases
    ...
    #            print mensaje.get_alpha()
                ...
                if globales.cuanto > texto_fondo + 30:
                    mensaje_flag = 1
    #                print "apago"
    #            if globales.cuanto > texto_fondo + 90:
    #                mensaje_flag = 2
    #                print "listo" 
    #            if globales.cuanto > texto_fondo + 121:
    #                mensaje_flag = 3

                if globales.cuanto > texto_fondo + 61:
                    mensaje_flag = 2
...
with window("Atención"):
    ...
    @button("Cómo fue?")
    def boton_rta():
        showInfo(message = (globales.pases, globales.numero))

    @button("Arrrrrrrrrrrrrrrrrrrranca!")
    def boton_arranca():
